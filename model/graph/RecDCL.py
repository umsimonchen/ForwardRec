...
# Original version - limited performance
...
class RecDCL_Encoder(nn.Module):
    ...
    def _init_model(self):
        ...
        embedding_dict = nn.ParameterDict({
            ...
            'u_target_his': nn.Parameter(initializer(torch.empty(self.data.user_num, self.latent_size)), requires_grad=False),
            'i_target_his': nn.Parameter(initializer(torch.empty(self.data.item_num, self.latent_size)), requires_grad=False),
        })
    ...
    def cal_cl_loss(self, user_idx, pos_idx):
        ...
        user_e, item_e_n = self.predictor(user_e_n), self.predictor(item_e_n)
...
# =============================================================================
# # Improved own version - better performance 
# 
# class RecDCL(GraphRecommender):
#     def __init__(self, conf, training_set, test_set):
#         super(RecDCL, self).__init__(conf, training_set, test_set)
...
#             'user_emb': nn.Parameter(initializer(torch.empty(self.data.user_num, self.latent_size))),
#             'item_emb': nn.Parameter(initializer(torch.empty(self.data.item_num, self.latent_size))),
#             'predictor': nn.Parameter(initializer(torch.empty(self.latent_size, self.latent_size))),
#             'projector_1': nn.Parameter(initializer(torch.empty(self.latent_size, self.latent_size))),
#             'projector_2': nn.Parameter(initializer(torch.empty(self.latent_size, self.latent_size))),
#             'projector_out': nn.Parameter(initializer(torch.empty(self.latent_size, self.latent_size))),
#             'u_target_his': nn.Parameter(initializer(torch.empty(self.data.user_num, self.latent_size)), requires_grad=False),
#             'i_target_his': nn.Parameter(initializer(torch.empty(self.data.item_num, self.latent_size)), requires_grad=False),
#         })
#         return embedding_dict
...
#     def bt(self, x, y):
#         for i in range(2):
#             x = torch.matmul(x, self.embedding_dict['projector_%d'%(i+1)])
#             x = nn.BatchNorm1d(self.latent_size).cuda()(x)
#             x = nn.ReLU(inplace=True)(x)
#             
#             y = torch.matmul(y, self.embedding_dict['projector_%d'%(i+1)])
#             y = nn.BatchNorm1d(self.latent_size).cuda()(y)
#             y = nn.ReLU(inplace=True)(y)
#         
#         user_e = torch.matmul(x, self.embedding_dict['projector_out'])
#         item_e = torch.matmul(y, self.embedding_dict['projector_out'])
#         c = nn.BatchNorm1d(self.latent_size, affine=False).cuda()(user_e).T @ nn.BatchNorm1d(self.latent_size, affine=False).cuda()(item_e)
#         c.div_(user_e.size()[0])
#         
#         # sum the cross-correlation matrix
#         on_diag = torch.diagonal(c).add_(-1).pow_(2).sum().div(self.latent_size)
...
#         
#         user_e = torch.matmul(x, self.embedding_dict['projector_out'])
#         xx = nn.BatchNorm1d(self.latent_size, affine=False).cuda()(user_e).T @ nn.BatchNorm1d(self.latent_size, affine=False).cuda()(user_e)
#         poly = (self.a * xx + self.c) ** self.e
#         return poly.mean().log()
#     
#     def loss_fn(self, p, z):  # cosine similarity
...
#         
#         user_e_n, item_e_n = torch.nn.functional.normalize(user_e, dim=-1), torch.nn.functional.normalize(item_e, dim=-1)
#         user_e = torch.matmul(user_e, self.embedding_dict['predictor'])
#         item_e = torch.matmul(item_e, self.embedding_dict['predictor'])
#         
#         # UIBT
#         if self.all_bt_coeff == 0.:
#             bt_loss = 0.
