...
class PreferDiff(GraphRecommender):
    def __init__(self, conf, training_set, test_set):
        ...
        self.model = PreferDiff_Encoder(mean_type, self.noise_schedule, self.noise_scale, self.noise_min, self.noise_max, self.steps, self.device)
    ...
    def train(self):
        ...
        self.full_prediction = torch.zeros(self.data.user_num, self.data.item_num).cuda()
        while not early_stopping :
            for n, batch in enumerate(next_batch_pairwise(self.data, self.batch_size)):
                user_idx, pos_idx, neg_idx = batch
                indices = torch.cat([torch.tensor(pos_idx).unsqueeze(1), torch.tensor(neg_idx).unsqueeze(1)], dim=1).cuda()
                
                batch_rating = TorchGraphInterface.convert_sparse_mat_to_tensor(self.data.interaction_mat[user_idx]).cuda().float().to_dense()
                term, model_output = model.training_losses(dnn_model, batch_rating, True)
                diff_loss = term['loss']
                
                score = torch.gather(model_output, dim=1, index=indices)
                bpr_loss = torch.mean(-torch.log(10e-6 + torch.sigmoid(torch.log(torch.maximum(score[:,0],eps)) - torch.log(torch.maximum(score[:,1],eps)))))
                total_loss = self.lamda * diff_loss.mean() + (1-self.lamda) * bpr_loss
                
                optimizer.zero_grad()
                total_loss.backward()
                optimizer.step()
                print('training:', epoch + 1, 'diffusion loss:',  self.lamda*diff_loss.mean().item(), 'bpr loss:', ((1-self.lamda)*bpr_loss).item())
            with torch.no_grad():
                prediction = model.p_sample(dnn_model, batch_rating, 0, False)
                self.full_prediction[user_idx] = prediction.detach()
            _, early_stopping = self.fast_evaluation(epoch)
            epoch += 1
        self.full_prediction = self.best_prediction
        with open('performance.txt','a') as fp:
            fp.write(str(self.bestPerformance[1])+"\n")

    def save(self):
        with torch.no_grad():
            self.best_prediction = self.full_prediction

    def predict(self, u):
        u = self.data.get_user_id(u)
        score = self.full_prediction[u]
